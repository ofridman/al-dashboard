var LibraryFileUploadManager = {
    ...
    $Unity_FileUploadManager: {
        ...
        jsSelectFilesCallback: null,
        jsUploadFinishedCallback: null,
        jsUploadProgressCallback: null,
        fileUploadQueue: [],
        filenameRegStr: ".*",
        descriptionStr: null,
        ...
        hide: function () {
            ...
            Unity_FileUploadManager.jsSelectFilesCallback = null;
        ...
        handleFileSelect: function (evt) {
            ...
            function fireSelectFilesCallback(results) {
                ...
                if (results != null) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        //This code was used to push directly to our server, which used to handle passing that file on to AWS S3, and storing a record in the database
                        // var formData = new FormData();
                        // formData.append("uploadedFile", file);
                        // axios.post(Unity_FileUploadManager.fileUploadQueue, formData, {
                        //         headers: {
                    ...
                    var selectedFiles = { "files": destFileArray };
                    var fileUploadDataJSON = JSON.stringify(selectedFiles);
                ...
                Runtime.dynCall('vi', Unity_FileUploadManager.jsSelectFilesCallback, [buffer]);
                _free(buffer);
            }
            ...
            Promise.resolve()
                    ...
                    return Promise.all(results.map(function (result) {
                        return new Promise(function (resolve, reject) {
                            ...
                            if (filenameReg.test(result.file.name)) {
                                //Commented this out as there is no point to load the file data into memory yet until unity messages us to upload the files
                ...
                .then(function (results) {
                    ...
                    console.log("fileUploadQueue: " + Unity_FileUploadManager.fileUploadQueue);
                    fireSelectFilesCallback(results);
        ...
        handleFileUpload: function () {
            ...
            function fireUploadProgressCallback(filename, progress) {

                if (!Unity_FileUploadManager.jsUploadProgressCallback) {
                    console.log("jsUploadProgressCallback null or undefined", Unity_FileUploadManager.jsUploadProgressCallback);
                    return;
                }

                console.log(filename);
                console.log(progress);
                var data = { "filename": filename, "progress": progress };

                console.log(data);

                var progressJSON = JSON.stringify(data);

                console.log(progressJSON);

                var returnStr = progressJSON;

                var size = lengthBytesUTF8(returnStr) + 1;
                var buffer = _malloc(size);
                stringToUTF8(returnStr, buffer, size);

                Runtime.dynCall('vi', Unity_FileUploadManager.jsUploadProgressCallback, [buffer]);
                _free(buffer);
            }

            function fireUploadFinishedCallback(filename) {

                if (!Unity_FileUploadManager.jsUploadFinishedCallback) {
                    console.log("jsUploadFinishedCallback null or undefined", Unity_FileUploadManager.jsUploadFinishedCallback);
                    return;
                }
                ...
                Runtime.dynCall('vi', Unity_FileUploadManager.jsUploadFinishedCallback, [buffer]);
                _free(buffer);
            }

            console.log("Beginning upload of files");
            console.log("fileUploadQueue = ", this.fileUploadQueue);
            // UPLOAD FILES

            if (!this.fileUploadQueue || this.fileUploadQueue.length === 0) {
                console.err("no files in the upload queue");
                return;
            }

            var fileUploadQueue = this.fileUploadQueue;

            Promise.resolve()
                .then(function () {
                    return Promise.all(fileUploadQueue.map(function (uploadEntry) {
                        console.log("uploadEntry: ", uploadEntry);
                        return new Promise(function () {
                            console.log("starting promise, reading file from disk");
                            var options = {
                                headers: {
                                    'Content-Type': uploadEntry.type
                                },
                                onUploadProgress: function (progressEvent) {
                                    fireUploadProgressCallback(uploadEntry.name, progressEvent.loaded / progressEvent.total);

                                }
                            };
                            axios.put(uploadEntry.signedUploadURL_S3, uploadEntry.file, options)
                                .then(function (res) {
                                    //console.log("file upload finished for: " + uploadEntry.name);
                                    console.log(res);
                                    //console.log(res.status);
                                    if (res.status === 200) {
                                        fireUploadFinishedCallback(uploadEntry.name);
                                    }
                                })
                                .catch(function (err) {
                                    console.error(uploadEntry.name + " failed to upload");
                                    console.error(err);
                                    fireUploadFinishedCallback(uploadEntry.name);
                                });
    ...
    Unity_FileUploadManager_PopupDialog: function (titleText, uploadBtnText, cancelBtnText) {
        ...
        Unity_FileUploadManager.jsUploadFinishedCallback = null;
        Unity_FileUploadManager.jsUploadProgressCallback = null;
    ...
    Unity_FileUploadManager_SetSelectFilesCallback: function (callback) {
        //console.log("Unity_FileUploadManager_SetCallback()");
        Unity_FileUploadManager.jsSelectFilesCallback = callback;
    },
    Unity_FileUploadManager_SetUploadFinishedCallback: function (callback) {
        console.log("Set upload finished callback");
        Unity_FileUploadManager.jsUploadFinishedCallback = callback;
    },
    Unity_FileUploadManager_SetUploadProgressCallback: function (callback) {
        console.log("Set upload progress callback");
        Unity_FileUploadManager.jsUploadProgressCallback = callback;
    },
    Unity_FileUploadManager_AddToUploadQueue: function (json) {
        var fileData = JSON.parse(Pointer_stringify(json));

        for (var i = 0; i < Unity_FileUploadManager.fileUploadQueue.length; i++) {
            if (Unity_FileUploadManager.fileUploadQueue[i].name === fileData.name) {
                console.log("existing fileData found: ", Unity_FileUploadManager.fileUploadQueue[i]);
                console.log("updating with values from: ", fileData);
                Unity_FileUploadManager.fileUploadQueue[i] = Object.assign({}, fileData, Unity_FileUploadManager.fileUploadQueue[i]);
                return;
            }
        }

        //Unity_FileUploadManager.fileUploadQueue.push(fileData);
        console.log("FILE NOT FOUND IN QUEUE " + json);
    },
    Unity_FileUploadManager_StartUploadingQueuedFiles: function () {
        Unity_FileUploadManager.handleFileUpload();
    },
