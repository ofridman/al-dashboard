    ...
    public class FileUploadExample : MonoBehaviour
    {
        ...
        void Start ()
        {
            ...
            if ( 
                ...
                WebGLFileUploadManager.SetDescription("Select image/video files (.png|.jpg|.mp4)");

            }else{
                WebGLFileUploadManager.Show (true);
                WebGLFileUploadManager.SetDescription("Drop image/video files (.png|.jpg|.mp4) here");
            }
            ...
            WebGLFileUploadManager.OnSelectedFileDataReceived += OnFileUploaded;
            WebGLFileUploadManager.OnFileUploadProgressUpdated += OnUploadProgressReceived;
        }
        ...
        private void OnFileUploaded(ImportedFileData[] result)
        {
            if(result.Length == 0) {
            ...
            foreach(ImportedFileData file in result){
                if(file.isSuccess){
        ...
        private void OnUploadProgressReceived(string filename, float progress)
        {
            Debug.Log($"UPLOAD PROGRESS RECEIVED: {filename} = {progress}");
        }
