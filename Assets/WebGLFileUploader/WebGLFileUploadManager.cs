...
namespace WebGLFileUploader
{
    ...
    public static class WebGLFileUploadManager
    {
        ...
        /// </summary>
        public static event Action<ImportedFileData[]> OnSelectedFileDataReceived;


        /// <summary>
        /// Fired when a js upload progress event is called
        /// </summary>
        public static event Action<string, float> OnFileUploadProgressUpdated;

        public static event Action<string> OnFileUploadFinished;
        ...
        public static bool Show (bool isDropInput = false, bool isOverlay = false, int x = -1, int y = -1, int width = -1, int height = -1)
        {
            ...
            WebGLFileUploadManager.Unity_FileUploadManager_SetSelectFilesCallback (WebGLFileUploadManager.SelectFilesCallback);
            WebGLFileUploadManager.Unity_FileUploadManager_SetUploadFinishedCallback(WebGLFileUploadManager.UploadFinishedCallback);
            WebGLFileUploadManager.Unity_FileUploadManager_SetUploadProgressCallback(WebGLFileUploadManager.UploadProgressCallback);
        ...
        {
            Debug.Log($"Showing FileUploader PopupDialog(titleText={titleText}, uploadBtnText={uploadBtnText}, cancelBtnText={cancelBtnText}");
#if UNITY_WEBGL && !UNITY_EDITOR
        ...
        [DllImport ("__Internal")]
        private static extern void Unity_FileUploadManager_SetSelectFilesCallback (Action<string> callback);

        [DllImport("__Internal")]
        private static extern void Unity_FileUploadManager_SetUploadProgressCallback(Action<string> callback);

        [DllImport("__Internal")]
        private static extern void Unity_FileUploadManager_SetUploadFinishedCallback(Action<string> callback);

        [DllImport ("__Internal")]
        private static extern void Unity_FileUploadManager_AddToUploadQueue(string json);

        [DllImport("__Internal")]
        private static extern void Unity_FileUploadManager_StartUploadingQueuedFiles();

        

        [AOT.MonoPInvokeCallback (typeof(Action<string>))]
        private static void SelectFilesCallback (string fileUploadDataJSON)
        {
            Debug.Log ("File Upload Callback called " + fileUploadDataJSON);

            if(OnSelectedFileDataReceived == null) {
                Debug.Log ("OnSelectedFileDataReceived == null");
                return;
            }

            ImportedFileData[] files;
            if (!string.IsNullOrEmpty(fileUploadDataJSON)) {
                files = JsonUtility.FromJson<FileUploadResult>(fileUploadDataJSON).files;
                OnSelectedFileDataReceived.Invoke (files);
            } else {
                files = new ImportedFileData[0]{};
                OnSelectedFileDataReceived.Invoke (files);
            }
                
            //Debug.Log (OnSelectedFileDataReceived);
        }

        [AOT.MonoPInvokeCallback (typeof(Action<string>))]
        private static void UploadProgressCallback (string uploadProgressJSON)
        {

            if(OnFileUploadProgressUpdated == null) {
                Debug.Log ("OnFileUploadProgressUpdated has no listeners");
                return;
            }

            Debug.Log ("UploadProgressCallback = " + uploadProgressJSON);

            if (!string.IsNullOrEmpty(uploadProgressJSON)) {

                FileUploadProgress uploadProgress = JsonConvert.DeserializeObject<FileUploadProgress>(uploadProgressJSON);

                OnFileUploadProgressUpdated.Invoke (uploadProgress.filename, uploadProgress.progress);
            } else {
                Debug.LogError("recieved upload progress callback for empty/null filename");
            }
        }

        [AOT.MonoPInvokeCallback (typeof(Action<string>))]
        private static void UploadFinishedCallback(string filename)
        {
            if(!string.IsNullOrEmpty(filename))
            {
                OnFileUploadFinished?.Invoke(filename);
            }
        }
        ...
        {
            public ImportedFileData[] files = new ImportedFileData[] {};
        }

        /// <summary>
        /// Pass a ImportedFileData object in as json after setting the signedAWSUploadUrl field
        /// </summary>
        /// <param name="json"></param>
        public static void AddToUploadQueue(string json)
        {
#if UNITY_WEBGL && !UNITY_EDITOR
            Unity_FileUploadManager_AddToUploadQueue(json);
#else
            Debug.LogError("Cant set file upload url in js from editor. Make a webgl build to test");
#endif
        }

        public static void StartUploadingQueuedFiles()
        {
#if UNITY_WEBGL && !UNITY_EDITOR
            Unity_FileUploadManager_StartUploadingQueuedFiles();
#else
            Debug.LogError("Doesnt work in the editor");
#endif
        }
    }
    ...
    public class ImportedFileData
    {
        ...
        public string signedUploadURL_S3 = "";
    }
